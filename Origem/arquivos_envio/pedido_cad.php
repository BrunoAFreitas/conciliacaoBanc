			...
			$query_pedant = mysql_query($sql_pedant,$conexao)or die("Erro na Busca do Codigo!");
			 ...
			 while($linha_pedant = mysql_fetch_object($query_pedant)){
               ...
               $msg_ped = "<table bgcolor='#004000' bordercolor='#004000' border='1' align='center'>
                            ...
                            Este Vendedor possui pedidos de venda [ ".$pedidos." ] em aberto ou fechado. Finalize-os primeiro!</font></b>
                            </td></tr></table>";
 			}else{
		   ...
		   //consultando seqpv no loja
			...
			if ($linha_pvautomatico0->lj_pvautomatico == "S"){
				  ...
				  if ($lstCli == ''){
                     $msg_ped = "<table bgcolor='#004000' bordercolor='#004000' border='1' align='center'>
                                  <tr><td align='center'><b><font size='3' color='#FFFF00'>
                                  Não pode gravar o cabeçalho do pedido sem escolher um cliente! Volte e refaça os procedimentos corretos!</font></b>
                                  </td></tr></table>";
			  ...
			  $query_tp = mysql_query($sql_tp,$conexao)or die("Erro na Busca do PV do Talao!");
               ...
               $msg_ped = "<table bgcolor='#004000' bordercolor='#004000' border='1' align='center'>
                            <tr><td align='center'><b><font size='3' color='#FFFF00'>
                            Pedido de Venda [ ".$edtNumPed." ] não foi cadastrado pelo gerente para este vendedor!</font></b>
                            </td></tr></table>";
              }
							...
							while ($linha_orcitens = mysql_fetch_object($query_orcitens)) {
							  ...
							  $sql_incl   = "INSERT INTO pedmov
											 (pm_num, pm_prod, pm_emp, pm_loja, pm_lojaloc, pm_cor, pm_escala,
											  ...
											  pm_incluido,pm_alterado,pm_dtincluido, pm_entregue,pm_comissao, pm_hora,pm_login)
											 VALUES 
											 ('$edtNumPed', '".$linha_orcitens->orc_prod."','$codemp', '$ljcod', '$ljcod', '".$linha_orcitens->orc_escala2."',
											  '".$linha_orcitens->orc_escala1."','".$linha_orcitens->orc_grupo."','S', '".$linha_orcitens->pro_descabv."',
											  '$pro_comple','".$linha_orcitens->orc_qtd."', '".$linha_orcitens->orc_valor."','$totaldoitem', 'S', 'N','$data','N',
											  '".$linha_orcitens->pro_comissao."', '$hora', '$acelogin')";
							  //echo $sql_incl;											  
...
<!-- NOVAS ALTERAÇÕES NA PAGINA INCLUINDO JQUERY E JAVASCRIPTY -->
 	<link rel="stylesheet" type="text/css" href="jquery.autocomplete.css" />
 	<link rel="stylesheet" type="text/css" href="estilomodal.css" />

	<script type="text/javascript" src="jquery.js"></script>
 	<script type="text/javascript" src="jquery.price_format.1.7.js"></script> 
 	<script type='text/javascript' src="jquery.autocomplete.js"></script>
 	<script type='text/javascript' src="functiontest.js"></script>
 	<script type='text/javascript' src="confimodal.js"></script>
<!-- NOVAS ALTERAÇÕES NA PAGINA INCLUINDO JQUERY E JAVASCRIPTY -->
...
<script language="JavaScript">
		...
		function troca(){
			 eval("popup('troca_prod.php?menuoff=<?=$menuoff?>&edtNumPed="+document.formpedido.edtNumPed.value+"',830,220,'center','center',POP_tot);");
		}	
...
<body onLoad="foco(<?=$campo?>)" topmargin="0" background="../imagens/fundomain.jpg" bottommargin="0" leftmargin="0" rightmargin="0"> 
<? //if ($menuoff != "ok"){
     include("menu_java.php");
   //}
?>
<form action="pedido_cad.php?incluir=s&campo=formpedido.edtEmissao&menuoff=<?=$menuoff?>" method="post" name="formpedido">
    ...
    <tr> 
      <td align="left" bgcolor="#085D44">
         ...
         <tr>
          ...
          <? if ($orc_num != ""){ ?>
	      <td align="right" bgcolor="#085D44"><font style="color:#FFF; font-size:18px">Orçamento <?=$orc_num?></font></td>
          <? } ?>          
         </tr>
        </table>
	 </td>
      ...
      <table bgcolor="#DFFFDF" width="100%" height="41%" border="0" cellpadding="2" cellspacing="0">
          ...
          <tr>
            <td>
				...
				?>
             <input name="lstCli" type="text" value="<?=$lstClirazao?>" style="font-weight:bold; border:solid 1; background-color:#FFFFC0; height:20; width:285;" onFocus="lookup(this.value);" />
             <!-- FOI RETIRADO O disabled DO CAMPO DE TEXTO PARA O AUTO COMPLETE FUNCIONAR
             	  E IMPLEMENTADO O METODO onFocus PARA O JS	-->
             
         <? if ($trava_cab != "disabled"){?>
			 
             <!-- ESTA CODIGO E UM LINK QUE CONTEM UMA IMAGEM QUE AO CLICKAR ABRE O MODAL -->
             	<a href="#janela1" rel="modal"><img src="mais.jpg" height="18px" width="18px" ></img></a>
             <!-- 
             	  ESTE BUTTON FOI COMENTADO PELO FATO DO MODAL SER EXECUTO EM LINKS
             
             <input name="Cliente" type="button" value="Pesq. Cliente" style="font-weight:bold; border:solid 1; background-color:#CCFFFF; height:20; width:80;" onClick="javascript:popup('pesq_cliente.php?tela=pedidocad&campo=formPesq.edtBusca&edtPedido=<?=$edtPedido?>&lstCli=<?=$lstCli?>&prod_estoque=s&incluir_prod=s&produtos=ok&trava=disabled&trava_list=disabled&edtNumPed=<?=$edtNumPed?>&edtSit=<?=$edtSit?>&trava_cab=<?=$trava_cab?>&lstVend=<?=$lstVend?>&lstTV=<?=$lstTV?>&edtEmissao=<?=muda_data_en($edtEmissao)?>menuoff=<?=$menuoff?>&orc_num=<?=$orc_num?>',850,550,'center','center',POP_tot);">
             -->
...
?>            
             <input name="edtNumPed" type="text" value="<?=$edtNumPed?>" size="8" maxlength="20" style="text-align:center; font-weight:bold; color:#000000; font-size:12; width:140; background-color: #FFFF80; border:solid 1;" <?=$trava_ped?>>
            </td>
            <? if ($edtEmissao == ""){
                   $edtEmissao = $data;
               } ?>
            <td>
             <input name="edtEmissao" type="text" style="font-weight:bold; border:solid 1; color:#000000; background-color:#FFFFC0;" size="11" value="<?=muda_data_pt($edtEmissao)?>" 
            ...
            <table width="100%" border="1" bordercolor='#00D500'>
                ...
                <tr> 
                  ...
                  <table bgcolor="#FFFFFF" width="100%" align="center" bordercolor='#008000' border="1">
                      ...
                      <?
								$sql_show = "SELECT DISTINCT pm_prod, pro_descabv, pm_cor,pm_loja, pm_escala, pro_foralinha,
                                                             pm_progrupo, pm_es, pm_qtd,
                                                             pm_valuni,pm_valtot
											 FROM pedmov, produtos
											 WHERE pro_cod = pm_prod AND pm_num = '$edtNumPed' AND pm_lojaloc = '$ljcod';";
								       // echo $sql_show;
